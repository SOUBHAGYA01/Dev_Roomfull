...
let PostList = () => {
 ...
 useEffect(() => {
  ...
  let submitCreatePost = async(e) => {
    ...
    {
      const {data} =  await axios.post("https://devroom-hfgx.onrender.com/api/posts/",localPost,{
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${localStorage.getItem("devroom")}`,
      },
  ...
  let clickDeletePost = async (postId) => {
    const { data } = await axios.delete(`https://devroom-hfgx.onrender.com/api/posts/${postId}`, {
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${localStorage.getItem("devroom")}`,
      },
